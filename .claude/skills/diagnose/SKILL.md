...
When exploring the codebase, use `docs/domain-glossary.md` to get a clear mental model of the relevant modules, and check the decision records in `docs/adr/` covering the area you're touching — a "bug" that is actually a recorded decision is a very expensive fix.
...
8. **Bisection harness.** If the bug appeared between two known states (commit, dataset, version), automate "boot at state X, check, repeat" so the version-control bisect tool can drive it.
9. **Differential loop.** Run the same input through old-version vs new-version (or two configs) and diff outputs.
10. **Human-in-the-loop script.** Last resort. If a human must click, drive _them_ with `.claude/skills/diagnose/scripts/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you.
