...
2. **Credentials live in spec maps, never in a URL string.** `books.db/db-spec` converts `DATABASE_URL` into a next.jdbc db-spec **map**; `datasource` and `migrate!` both take that map. **Binding rule: no code in this repo may construct a JDBC URL containing a user or password.** A reviewer seeing `:jdbcUrl` with credentials in it should treat it as a defect, not a style choice.
   ...
   > **Amendment, 2026-08-10 — the rule holds for the Google Books API key too.** This clause was written about the database, and the Google Books adapter (`books.google-books`) claimed exemption from it on the grounds that "the key travels in the URL". That premise was false, and it was checked rather than argued: the Books API accepts the key as an `X-goog-api-key` **request header** and honours it identically to `?key=`. Verified against the live API — a bogus key answers `400 INVALID_ARGUMENT "API key not valid"` through *both* forms (the key-validation path), while sending no key at all answers `429` (the anonymous-quota path), so the header genuinely authenticates rather than being ignored. The `key=` query parameter is therefore gone: `search-url` builds a URL with no credential in it, and `http-fetch` sends the header. **The binding rule now reads: no code in this repo may put any credential — database or API key — into a URL string.** This narrows and clarifies the same decision; it reverses nothing, so it is recorded here rather than in a new ADR (see the INDEX conventions). Two consequences follow and are kept deliberately: redirects are still never followed (the JDK's redirect filter replays custom headers onto the new request, so a followed redirect would hand the key to the redirect's origin), and `redact` survives as a second line for a diagnostic built from a message we did not construct, not as the primary control it used to be.
