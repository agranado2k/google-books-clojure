...
| Bump the vendored htmx              | `scripts/vendor-htmx.sh` — fetches the release pinned in `src/books/assets.clj` (its only copy of the version + SHA-256); the suite re-hashes the bytes every run |
